...
Order robots from RobotSpareBin Industries Inc
    TRY
        ...
        ${CONFIRM}=    Confirmation dialog    https://robotsparebinindustries.com/orders.csv
        IF    ${CONFIRM} == ${True}
            ${orders}=    Get orders
            FOR    ${row}    IN    @{orders}
                Close the pop up
...
Dialog as progress indicator
    ...
    ${dialog}=    Show dialog    title=Please wait    on_top=${TRUE}    height= 280
...
Confirmation dialog
    [Arguments]    ${url}
    Add icon    Success
    Add heading    Confirm the download link: ${\n}${url}
    Add submit buttons    buttons=No,Yes    default=Yes
    ${result}=    Run dialog    title=Confirmation    height= 480    on_top=${TRUE}
    IF    $result.submit == "Yes"
        ${CONFIRM}=    Set Variable    ${True}
        RETURN    ${CONFIRM}
    ELSE
        Close Browser
    END
...
Take a screenshot of the robot
    ...
    RPA.Browser.Selenium.Screenshot    css:div#robot-preview-image    ${screen}
    RETURN    ${screen}
